@extends('layouts.master', ['title' => 'مدیریت قرارداد ها'])
@section('content')
    ...
    <div class="kt-subheader   kt-grid__item" id="kt_subheader">
        <div class="kt-container">
            <div class="kt-subheader__main">
                ...
                <span class="kt-subheader__desc iranyekan">مدیریت -> محدوده و قراردادها -> قرارداد ها</span>
            </div>
            <span  class="badge badge-primary iranyekan reportDate">تاریخ آخرین بروزرسانی گزارش: {{ jdate($contracts->sortByDesc('updated_at')->first()->updated_at)->format('Y/m/d') }}</span>
                ...
                <div class="kt-portlet kt-portlet--height-fluid">
                    ...
                    <div class="kt-portlet__body">
                        <table style="font-family: iranyekan; width: 100%; direction: rtl!important; " class="table table-striped table-bordered table-hover table-checkable display " id="m_table_3">
                            ...
                            <tr>
                                <th>ردیف</th>
                                <th>شماره قرارداد/ الحاقیه </th>
                                <th>وضعیت قرارداد</th>
                                <th>طرف قرارداد/ پیمانکار</th>
                                <th>عنوان قرارداد</th>
                                <th>مبلغ اولیه ریالی</th>
                                <th>مبلغ اولیه ارزی</th>
                                <th>مبلغ نهایی قراداد با احتساب 25 % و الحاقیه ها  ریالی</th>
                                <th>مبلغ نهایی قراداد با احتساب 25 % و الحاقیه ها  ارزی</th>
                                <th>تاریخ عقد قرارداد</th>
                                <th>تاریخ ابلاغ</th>
                                <th>شماره ابلاغ</th>
                                <th>تاریخ شروع</th>
                                <th>مدت اولیه قرارداد</th>
                                <th>نوع</th>
                                <th>تاریخ اتمام</th>
                                <th>مدت باقیمانده تا پایان قرارداد</th>
                                <th> آخرین صورت وضعیت شماره </th>
                                <th>ریالی</th>
                                <th>ارزی</th>
                                <th>درصد پیشرفت مالی</th>
                                <th>تضمین انجام تعهدات</th>
                                <th>تضمین پیش پرداخت</th>
                                <th>تضمین حسن انجام کار</th>
                                <th>مجوز فعالیت تاریخ</th>
                                <th>مجوز فعالیت شماره</th>
                                <th>تاریخ تحویل موقت</th>
                                <th> مهلت رفع نقص</th>
                                <th>تاریخ رفع نقص</th>
                                <th>مدت دوره تضمین(ماه)</th>
                                <th>تاریخ تحویل قطعی </th>
                                <th> وضعیت مفاصا حساب بیمه</th>
                                <th> آدرس</th>
                                <th> مدیر عامل</th>
                                <th> نماینده</th>
                                <th>همراه</th>
                                ...
                                <th>ایمیل</th>
                                <th> بالانس متریال</th>
                                <th> صورتحساب نهایی</th>
                                <th>امور نیاز به اقدام / در دست اقدام</th>
                                <th>نوع</th>
                                <th>میلادی</th>
...
{{--                                <tr {{ $tender->sherkatBarande != null  ? 'style=background-color:#c8f7c561' : ''}}>--}}
                                <tr>
                                    ...
                                    <td>{{ $contract->shomare }}</td>
                                    <td>{{ $contract->vaziat }}</td>
                                    <td>{{ $contract->taraf }}</td>
                                    <td>{{ $contract->onvan }}</td>
                                    <td>{{ $contract->riali }}</td>
                                    <td>{{ $contract->arzi }}</td>
                                    <td>{{ $contract->mablaghNahaeiRiali }}</td>
                                    <td>{{ $contract->mablaghNahaeiRialiArzi }}</td>
                                    <td>{{ $contract->tarikhAghd }}</td>
                                    <td>{{ $contract->tarikhEblagh }}</td>
                                    <td>{{ $contract->shomareEblagh }}</td>
                                    <td>{{ $contract->tarikhShoroo }}</td>
                                    <td>{{ $contract->moddatAvvalie }}</td>
                                    <td>{{ $contract->dateType }}</td>
                                    <td>{{ $contract->tarikhEtmam }}</td>
                                    <td>{{ $contract->moddatBaghimande }}</td>
                                    <td>{{ $contract->shomareSooratvaziat }}</td>
                                    <td>{{ $contract->mablaghSootarvaziatRiali }}</td>
                                    <td>{{ $contract->mablaghSootarvaziatArzi }}</td>
                                    <td>{{ $contract->darsadPishraftMali }}</td>
                                    <td>{{ $contract->tazminAnjam }}</td>
                                    <td>{{ $contract->tazminPishpardakht }}</td>
                                    <td>{{ $contract->tazminHosn }}</td>
                                    <td>{{ $contract->mojavvezFaliatTarikh }}</td>
                                    <td>{{ $contract->mojavvezShomare }}</td>
                                    <td>{{ $contract->tahvilMovaghatTarikh }}</td>
                                    <td>{{ $contract->mohlatRafNaghs }}</td>
                                    <td>{{ $contract->tarikhRafNaghs }}</td>
                                    <td>{{ $contract->moddatDoreTazmin }}</td>
                                    <td>{{ $contract->tahvilGhatiTarikh }}</td>
                                    <td>{{ $contract->mafasa }}</td>
                                    <td>{{ $contract->address }}</td>
                                    <td>{{ $contract->modiramel }}</td>
                                    <td>{{ $contract->namayande }}</td>
                                    <td>{{ $contract->mobile }}</td>
                                    <td>{{ $contract->tel }}</td>
                                    <td>{{ $contract->fax }}</td>
                                    <td>{{ $contract->email }}</td>
                                    <td>{{ $contract->balanceMaterial }}</td>
                                    <td>{{ $contract->sooorathesabNahaei }}</td>
                                    <td>{{ $contract->omoorPending }}</td>
                                    <td>{{ $contract->type }}</td>
                                    <td>{{ $contract->miladi }}</td>
                                    <td style="font-family: BYekan!important; direction: ltr!important;">{{ jdate($contract->updated_at) }}</td>
                                    <td style="text-align:center;" > <a href="{{ route('contracts.edit', $contract->id) }}" ><i style="font-size: 18px;" class="fa fa-edit"></i></a>  </td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
